<template>
  <div class="index_main" v-if="showMe">
    ...
    <div class="index_header">
      ...
      <!-- 热搜词 -->
      <div class="hot_word">
        <router-link v-for="x in getFalseHotWord" :to="'/search/' + x.search_word" :key=getFalseHotWord[x]>
          <span>{{ x.title }}</span>
        </router-link>
      </div>
    </div>
    ...
    <!-- 推荐商家列表 -->
    <div>
      <OneBusiness v-for="n in getFalseBussinessbrief" :a="n" :key=getFalseBussinessbrief[n]></OneBusiness>
    </div>
    <!-- 撑开Fixednav挡住的位置 -->
    <div class="space"></div>
    <!-- 固定导航栏 -->
